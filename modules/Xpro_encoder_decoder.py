...
class Xpro_EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer, mode = 'train'):
        super(Xpro_EncoderDecoder, self).__init__(args, tokenizer)
        self.args = args
        ...
        self.cmn = MultiThreadMemory(args.num_heads, args.d_model, topk=self.topk)

        self.model = self.make_model(tgt_vocab, self.cmn)
        self.logit = nn.Linear(args.d_model, tgt_vocab)

        self.protypes = nn.Parameter(torch.FloatTensor(self.num_protype * self.num_cluster, args.d_txt_ebd + args.d_img_ebd))
        if mode == 'train':
