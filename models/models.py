...
from modules.Xpro_encoder_decoder import Xpro_EncoderDecoder
from modules.Xpro_visual_extractor import Xpro_VisualExtractor
...
class BaseCMNModel(nn.Module):
    def __init__(self, args, tokenizer):
        ...
        self.Xpro_visual_extractor = Xpro_VisualExtractor(args)
        self.encoder_decoder = BaseCMN(args, tokenizer)
        self.Xpro_encoder_decoder = Xpro_EncoderDecoder(args, tokenizer)
    ...
    def forward_iu_xray(self, 
        ...
        Xro_att_feats_0, Xpro_fc_feats_0 = self.Xpro_visual_extractor(images[:, 0])
        Xro_att_feats_1, Xpro_fc_feats_1 = self.Xpro_visual_extractor(images[:, 1])
        Xpro_fc_feats = torch.cat((Xpro_fc_feats_0, Xpro_fc_feats_1), dim=1)
        xpro_att_feats = torch.cat((Xro_att_feats_0, Xro_att_feats_1), dim=1)

        if mode == 'train':
            Xpro_output = self.Xpro_encoder_decoder(Xpro_fc_feats, xpro_att_feats, targets, labels=labels, mode='forward')
            text_embeddings, output = self.encoder_decoder(fc_feats, att_feats, targets, mode='forward')
            return Xpro_output, output, mask_feats, text_embeddings, disease_token_target, disease_token_feats
        elif mode == 'sample':
